      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <ScrollReveal>
          <div className="text-center mb-8 sm:mb-12">
            <span className="text-[10px] sm:text-xs font-bold tracking-widest uppercase text-accent mb-2 sm:mb-3 block">
              {t("sectors.label")}
            </span>
            <h2 className="text-2xl sm:text-4xl font-heading text-foreground mb-3 sm:mb-4">
              {t("sectors.title")}
            </h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {sectors.map((sector, i) => (
            <ScrollReveal key={sector.key} delay={i * 120} direction="up" distance={50}>
              ...
              >
                <div className={`w-12 h-12 sm:w-14 sm:h-14 rounded-xl ${sector.color} flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-300`}>
                  <sector.icon className="w-6 h-6 sm:w-7 sm:h-7" />
                </div>
