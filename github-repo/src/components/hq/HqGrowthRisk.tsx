  ...
  return (
    <div className="bg-card border border-border p-8 rounded-2xl h-full flex flex-col shadow-sm">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h3 className="text-xl font-heading font-bold text-foreground tracking-tight">Churn Sentinel</h3>
          <p className="text-sm text-muted-foreground mt-1">Sistemi terk etme riski olan işletmeleri erkenden tespit et.</p>
        </div>
      ...
      <div className="space-y-4 flex-1">
        {data.map((merchant, i) => (
          <div key={i} className="flex items-center justify-between p-4 bg-muted/30 border border-border rounded-xl hover:bg-muted/50 transition-all group">
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 rounded-full bg-muted border border-border flex items-center justify-center font-bold text-muted-foreground group-hover:bg-primary/20 group-hover:text-primary transition-colors">
                {merchant.name[0]}
              </div>
              <div>
                <h4 className="text-sm font-semibold text-foreground">{merchant.name}</h4>
                <p className="text-[11px] text-muted-foreground">Son Hareket: {merchant.lastActive}</p>
              </div>
            </div>
